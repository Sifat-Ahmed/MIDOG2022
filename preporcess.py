...
def main(cfg):
    ...
    for i, data in enumerate(tqdm(coco_images, colour='green', leave=True, position=0)):
        ...
        for j in range(len(bbox)):
            ...
            #
            #     while (flag == 1):
            #         x1 = random.randint(0, image_w - cfg.size.patch)
            #         y1 = random.randint(0, image_h - cfg.size.patch)
            #         for cell in bbox:
            #             xx = int(box[0])
            #             yy = int(box[1])
            #             if (xx > x1 + 512 or xx < x1 - 50 or y1 > y1 + 512 or y1 < y1 - 50): flag = 0
            #     image_index += 1
            #     slice = img[y1:y1 + 512, x1:x1 + 512]
            #
            #     images.append(
            #         {
            #             "license": 1,
            #             "file_name": '{}_{}_{}_empty.png'.format(i + 1, x1, y1),
            #             "id": image_index,
            #             "width": cfg.size.patch,
            #             "height": cfg.size.patch
            #          }
            #     )
            #     cv2.imwrite(os.path.join(cfg.write_dir.image, '{}_{}_{}_empty.png'.format(i + 1, x1, y1)), slice)
